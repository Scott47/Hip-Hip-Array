...
export const getStudentsInCohort = (cohort) => {
   const thisCohort = students.filter(student => student.cohort === cohort)
   return thisCohort
}
...
export const getStudentsByInstructorId = (instructorId) => {
    const studentsByInstructor = students.filter(student => student.instructorId === instructorId)
...
export const getPolyglotStudents = (languageCount) => {
    const studentsLanguageCount = students.filter(student => student.languages.length >= languageCount)
    return studentsLanguageCount
}
...
export const getAvailableInstructors = () => {
    const availableInstuctors = instructors.filter(i => 
      !students.find(s => s.instructorId === i.id)
    )
...
// export const getStudentsByLangugages = (languages) => {
//     languages.map(l => {
//        let home =  students.every(student => student.languages.includes(l))
//        console.log(home)
//     })
        // console.log(dudes)
}
